...
@pure def splitOrFallback(string: String, splitter: C => B @pure, fallback: String): String = {
  val split = ops.StringOps(string).split(splitter)
...
//val excludedMatches: ISZ[Os.Path] = ISZ(bin, out, build) // hidden dirs are also ignored
...
val modules = home.list
  .filter(isModule)
  .map(pathShortName)
  .filter((name: String) => ops.StringOps(name).indexOf(c".") != z"0")

modules.foreach((shortName: String) => {
  run(ISZ("sireum", "proyek", "test", ".", shortName))
})